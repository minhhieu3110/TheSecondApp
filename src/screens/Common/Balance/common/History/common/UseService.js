import {Block, Text, Image, StatusBar, ScrollView, NoneData} from '@components';
...
import {walletLog} from 'redux/reducers/combineReducers/userReducers';
export default function UseService() {
  const dispatch = useDispatch();
  useEffect(() => {
    dispatch({
      type: actions.WALLET_LOG,
      params: {type: 'buy_service'},
    });
  }, [dispatch]);
  const useLog = useSelector(state => state.walletLog?.data || []);
